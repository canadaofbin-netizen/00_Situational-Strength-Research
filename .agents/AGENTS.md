...
- 모든 지식 노드(마크다운 파일의 제목 및 본문)는 반드시 글로벌 스탠다드인 **100% 영어(English)**로만 작성되어야 합니다.
- 한국어 프롬프트가 주어지더라도, 백그라운드에 소환되는 서브 에이전트들을 포함한 모든 에이전트들은 지식망 데이터를 기록할 때 한국어 번역이나 혼용을 절대 금지하며 영어를 기본 언어로 사용해야 합니다.
